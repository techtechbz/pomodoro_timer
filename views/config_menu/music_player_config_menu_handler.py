...
class DirectToMusicAppViewHandler:
    def __init__(self, scroll_view_width, frame_height):
...
class PlaylistConfigItemHandler:
    def __init__(self, playlist_name, item_name):
        ...
        self.__playlist_config_item_view.border_color = "#ffffff"
        self.__playlist_config_item_view.border_width = 1

    def get_view_instance(self):
        return self.__playlist_config_item_view

    def set_select_button_action(self, action):
        self.__playlist_config_item_view["select_button"].action = action

    def style_appearance(self, height, y, default_tint_color, background_color):
...
class PlaylistConfigRadioGroupAppearanceSettings:
    def __init__(self):
        self.__item_height = 40
        self.__even_item_background_color = "#cccccc"
        self.__odd_item_background_color = "#eeeeee"
        self.__selected_button_tint_color = "#4f8aff"
        self.__unselected_button_tint_color = "#bbbbbb"

    def return_item_height(self):
        return self.__item_height

    def calculate_scroll_contents_height(self, item_number):
        return self.__item_height * item_number

    def calculate_item_position_vertical_axis(self, order):
        return self.__item_height * order

    def distinguish_select_button_tint_color(self, is_selected):
        if is_selected:
            return self.__selected_button_tint_color
        else:
            return self.__unselected_button_tint_color

    def distinguish_item_background_color(self, order):
        if order % 2 == 0:
...
class PlaylistConfigRadioGroupHandler:
    ...
    @staticmethod
    def get_playlist_config_item_name(order):
        return f"playlist_{order}"

    def get_available_playlist(self):
    ...
    def insert_scroll_view_contents(self):
    ...
    def setting_appearance_of_group_ratio(self):
    ...
    def insert_direct_to_music_app_sentence_view(self):
    ...
    def insert_playlist_config_item(self):
    ...
    def unselect_current_playlist(self):
        if self.__selected_playlist_name != "":
            previous_selected_button = self.__scroll_view[self.__available_playlist[self.__selected_playlist_name]][
                "select_button"]
            previous_selected_button.tint_color = self.__appearance_setting.distinguish_select_button_tint_color(False)

    def set_selected_playlist_name(self, updated_playlist_name):
    ...
    def switch_selected_playlist_name(self, sender):
        self.unselect_current_playlist()
...
class MusicPlayerConfigMenuHandler:
    def __init__(self, current_music_player_settings):
        ...
        self.__playlist_config_group_raito_handler = PlaylistConfigRadioGroupHandler(
            self.__view_instance["select_playlist_scroll_view"],
            self.__current_music_player_settings["playlist_name"]
        )
        self.__playlist_config_group_raito_handler.insert_scroll_view_contents()

    def get_view_instance(self):
        return self.__view_instance

    def reset_settings(self, music_player_settings):
    ...
    def write_out_field_inputs(self):
        ...
        self.__current_music_player_settings["playlist_name"] = \
            self.__playlist_config_group_raito_handler.get_selected_playlist_name()
    ...
    def insert_field_text(self):
        ...
        self.__view_instance["will_play_music_on_break_switch"].value = self.__current_music_player_settings[
            "will_play_music_on_break"]
        self.__playlist_config_group_raito_handler.set_selected_playlist_name(
            self.__current_music_player_settings["playlist_name"])
